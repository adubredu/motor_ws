...
######################################################
    ...
    def __init__(self):
    #####################################################
        rospy.init_node("r_pid_velocity")
        ...
        self.out_min = rospy.get_param('~out_min',-60)
        self.out_max = rospy.get_param('~out_max',60)
        self.rate = rospy.get_param('~rate',30)
        self.rolling_pts = rospy.get_param('~rolling_pts',2)
        self.timeout_ticks = rospy.get_param('~timeout_ticks',4)
        self.ticks_per_meter = rospy.get_param('ticks_meter', 174)
        self.vel_threshold = rospy.get_param('~vel_threshold', 1)
        self.encoder_min = rospy.get_param('encoder_min', -500000)
        self.encoder_max = rospy.get_param('encoder_max', 500000)
        ...
        rospy.Subscriber("r_wheel_enc", Int32, self.wheelCallback) 
        rospy.Subscriber("r_wheel_vtarget", Float32, self.targetCallback) 
        self.pub_motor = rospy.Publisher('r_motor_cmd',Float32, queue_size=10) 
        self.pub_vel = rospy.Publisher('r_wheel_vel', Float32, queue_size=10)
